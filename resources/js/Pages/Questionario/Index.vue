  ...
  <AuthenticatedLayout>
    <div class="row p-3">
      ...
      <div class="col-sm-6 fw-medium justify-content-end d-flex">
        <button @click="paginaNovoQuestionario" class="btn-padrao">+ Novo questionário</button>
      </div>
    </div>

    <di class="row p-3">
      <div class="col-sm-11">
        <input @input="apiQuestionarios" v-model="busca.questionario" type="text" class="form-control" placeholder="Pesquisar">
      </div>
      <div class="col-sm-1 justify-content-end d-flex">
        <button @click="apiQuestionarios" class=" fs-4"><i class="bi bi-search"></i></button>
      </div>
    </di>
    ...
    <div class="row p-3">
      <div v-for="questionario in questionarios" style="height: 500px; width: 450px;"
        ...
        <div>
          <div class="row mb-2">
            <div class="col-sm-9 justify-content-start d-flex align-items-center rounded fw-medium fs-5 text-center">
            ...
            <div class="col-sm-3 justify-content-end d-flex">
              <button class="btn fs-2" data-bs-toggle="dropdown" aria-expanded="false">
                <i class="bi bi-three-dots"></i>
              </button>

              <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                <li @click="deletarQuestionario(questionario.id)"><a class="dropdown-item" href="#">Deletar</a></li>
                <!--
        ...
        <div>
          ...
          <div class="row">
            <div @click="verQuestionario(questionario.id)" class="btn btn-outline-secondary fw-bold text-black fs-5">
              Analisar os resultados</div>
          </div>
        </div>
      </div>
    </div>
              ...
              <div v-if="novoQuestionario.perguntas.length">
                <div class="row mt-5">
                  <div class="col-sm-12 mt-5">
                    <div v-for="(pergunta, index) in novoQuestionario.perguntas" :key="pergunta.id" class="row mb-3">
                      <div class="col-sm-12">
                        <div class="row mb-3">
                          <div class="col-sm-10 fs-3 fw-bold">{{ pergunta.nome }}</div>
                          <div class="col-sm-2">
                            <button @click.prevent="deletarPergunta(index)" class="btn btn-danger ms-2"><i
  ...
  methods: {
    apiQuestionarios() {
      axios.post(route('questionario.api', {
        questionario: this.busca.questionario
      }))
        .then((response) => {
    ...
    paginaNovoQuestionario() {
      this.$inertia.get(route('questionario.novo'));
    },
    deletarQuestionario(id){
      this.$inertia.get(route('questionario.deletar', { id }));
    }
  }
});
</script>
